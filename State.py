...
class State(QutipWrapper):
    @staticmethod
    def _create_basis(number_of_fock_states: int, *args, **kwargs):
        return qutip.basis(number_of_fock_states, *args, **kwargs)

    @staticmethod
    def _create_coherent(number_of_fock_states: int, *args, **kwargs):
        return qutip.coherent(number_of_fock_states, *args, **kwargs)

    @staticmethod
    def _create_pegg_barnett(number_of_fock_states: int, angle_in_radians: float = 0):
    ...
    creation_functions = {
        'basis': _create_basis,
        'coherent': _create_coherent,
        'pegg-barnett': _create_pegg_barnett
    }
    ...
    def unit(self):
        return self.__class__(super().unit())
    ...
    @staticmethod
    def mesolve(hamiltonian: qutip.Qobj, state: State, t_list: list[float], collapse_operators: list[qutip.Qobj]):
        return list(map(
            State,
            qutip.mesolve(hamiltonian, state, tlist=t_list, c_ops=collapse_operators).states))
